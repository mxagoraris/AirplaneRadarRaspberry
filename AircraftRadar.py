...
def save_display_image(image):
    """Save image for display."""
    try:
        image.save("/tmp/flight_display.ppm", "PPM")
    except Exception as e:
        pass  # Silent fail for display-only feature
...
def main():
    ...
    while True:
        ...
        if raw_list is None:
            ...
            create_display_image(aircraft)
            
            # Save image for LED matrix
            if aircraft:
                image = create_display_image(aircraft)
                save_display_image(image)
